---
title: "MSIClust Vignette"
author: "MSI clustering workflow"
...
This vignette shows how to run **MSIClust** on an MSI pixel data frame. It only
covers the MSIClust workflow and the helper functions needed to prepare input,
run the clustering, inspect the fuzzy memberships, and plot the result.

If you are viewing this on GitHub, use the rendered
[`MSIClust_vignette.md`](MSIClust_vignette.md) file. The figures used by the
rendered vignette are stored in `MSIClust_vignette_files/figure-gfm/`.
...
For an existing MSI pixel data frame with `x`, `y`, and `mz_...` columns, the
core workflow is:
...
The sections below walk through the same workflow in more detail, including a
synthetic imzML example.

# What MSIClust Does

MSIClust is used here as a spatially adaptive fuzzy clustering workflow for MSI
pixels. It runs fuzzy c-means through `vsclust`, but gives each pixel its own
fuzzifier based on how similar that pixel is to its local neighbors.

The core idea is:

1. Each pixel has an intensity spectrum across `mz_` features.
2. Neighboring pixels are compared by spectral correlation.
3. Local disagreement is converted to `inv_cor = 1 - avg_corr_neighbors`.
4. `inv_cor` is scaled by `cor_scale`.
5. `vsclust::determine_fuzz()` converts the scaled values into per-pixel
   fuzzifiers.
...
```{r cardinal-packages, eval=FALSE}
library(Cardinal)
library(BiocParallel)
```

The included synthetic imzML example was generated with `CardinalIO`; you only
need `CardinalIO` if you want to regenerate those example files.
...
The `mz_` columns are the clustering features. Other metadata columns can be
present, but the core helper functions look for `x`, `y`, and `mz_...`.

# Load Helpers

Start R from the repository root: the paths in this vignette are relative to it.

```{r load-helpers, eval=FALSE}
script_root <- normalizePath(".", mustWork = FALSE)
...
If you already have `msi_df`, skip this section. If you have a binned Cardinal
MSI object, convert it with `make_msi_dataframe()`. Attach Cardinal first with
`library(Cardinal)`; calling it only as `Cardinal::...` is not enough for
`make_msi_dataframe()`.

```{r make-msi-df, eval=FALSE}
...
This vignette uses a compact synthetic imzML/ibd pair generated specifically for
demonstrating MSIClust. The files are in
`example_data/Synthetic_MSIClust/`.

The synthetic data contain a 45 x 35 pixel image with 120 m/z features. Four
spatial regions were simulated with shared peaks, region-specific peaks, smooth
spatial transitions, pixel-level scaling, and noise. The synthetic region labels
are included only to show how the example was made; MSIClust does not use them.

To regenerate the example files:

```{r regenerate-synthetic-example, eval=FALSE}
system2(
  "Rscript",
  file.path(
    script_root,
...
```{r synthetic-example-design-plot}
ggplot(
  synthetic_design,
...
```{r synthetic-example-diagnostics}
...
`plot_msiclust_map()` accepts either a result-like list with a `clustered_df`
element or a plain data frame. For MSIClust-only use, pass the data frame and
...
If preparation fails because no valid neighbor correlations were found, check
that `x` and `y` form the expected pixel grid and that the `mz_` columns are
numeric.

```{r no-neighbors, eval=FALSE}
signal_cols <- msiclust_signal_columns(msi_df)
...
If many pixels become `No_cluster`, inspect the membership distribution. You may
need a lower `min_membership`, a different `cor_scale`, or a different number of
clusters.

```{r too-many-no-cluster, eval=FALSE}
hist(msiclust_res$max_membership, breaks = 100)
mean(msiclust_res$labels == "No_cluster")
```
...
The helpers expect features named like `mz_369.3`. If your columns use another
prefix, rename them or pass `signal_cols` explicitly to the helpers that support
it.
